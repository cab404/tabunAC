...
public class SmilesPart extends Part {
    ...
    public SmilesPart(SmileHandler handler) {
        ...
        JSONArray def_smiles = new JSONArray();
        for (String s : Static.ctx.getResources().getStringArray(R.array.default_smiles))
            def_smiles.add(s);

        JSONArray raw_smiles = Static.obscure.ensure("editor.smiles", def_smiles);
